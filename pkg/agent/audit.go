...
func newAuditWriter(
	...
	if err := os.MkdirAll(userRoot, 0700); err != nil {
		logger.Errorf("Create agent audit directory failed: %s", err)
		return nil
	}
	...
	if err != nil {
		logger.Errorf("Open agent audit file failed: %s", err)
		return nil
	}
	_ = file.Chmod(0600)
	return &auditWriter{
...
func (w *auditWriter) Record(event string, payload any) {
	if event == "provider_fallback" || event == "context_fallback" {
		...
		logger.Infof("Agent AI %s: %s", event, value)
	}
...
func (w *auditWriter) SetSessionID(value string) {
	...
	if err := os.Rename(w.tempPath, path); err != nil {
		logger.Errorf("Rename agent audit file failed: %s", err)
	...
	if err != nil {
		logger.Errorf("Reopen agent audit file failed: %s", err)
		w.file = nil
		return
	}
	w.file = file
...
func (w *auditWriter) Write(event string, payload any) {
	...
	if _, err = w.file.Write(append(value, '\n')); err != nil {
		logger.Errorf("Write agent audit file failed: %s", err)
	}
}
...
func pruneAuditSessions(root string, retention int) {
	...
	for _, file := range files[min(retention, len(files)):] {
		...
		if err := os.Remove(file.path); err != nil {
			logger.Errorf("Prune agent audit file failed: %s", err)
		}
	}
}
